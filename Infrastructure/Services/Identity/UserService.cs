...
namespace Infrastructure.Services.Identity
{
    public class UserService : IUserService
    {
        ...
        public UserService(UserManager<ApplicationUser> userManager, RoleManager<ApplicationRole> roleManager, IMailService mailService, IExportService exportService, ICurrentUserService currentUserService, IMapper mapper)
        {
        ...
        public async Task<IResult<string>> ConfirmEmailAsync(string userId, string code)
        {
            ...
            if (result.Succeeded)
            {
                return await Result<string>.SuccessAsync(user.Id, 
                    string.Format("Account Confirmed for {0}. You can now use the /api/identity/token endpoint to generate JWT.", user.Email));
            }
            else
            {
        ...
        public async Task<Result<string>> ExportToExcelAsync(string searchString = "")
        {
            ...
            if(users.Count > 0)
            {
                var result = await _exportService.ExportToExcelAsync(users, sheetName: "Users",
            ...
            return await Result<string>.FailAsync("No Users Found For Export.");
        }
        ...
        public async Task<Result<List<UserResponse>>> GetAllAsync()
        {
            ...
            return await Result<List<UserResponse>>.FailAsync("No Users Found.");
        }
        ...
        private async Task<string> SendVerificationEmail(ApplicationUser user, string origin)
        {
            ...
            code = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(code));
            var route = $"api/v{1}/identity/Users/confirm-email/";
            var endpointUri = new Uri(string.Concat($"{origin}/", route));
        ...
        public async Task<IResult> UpdateRolesAsync(UpdateUserRolesRequest request)
        {
            ...
            return await Result.FailAsync($"Failed Updating Roles for User: {request.UserId}");
        }
    }
}
