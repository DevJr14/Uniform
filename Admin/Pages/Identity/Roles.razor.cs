    ...
    public partial class Roles
    {
        ...
        private async Task InvokeModal(string id = null)
        {
            ...
            if (id != null)
            {
                ...
                if (_role != null)
                {
                    parameters.Add(nameof(AddEditRoleModal.RoleModel), new RoleRequest
                    {
                        Id = _role.Id,
                        Name = _role.Name,
            ...
            var options = new DialogOptions { CloseButton = true, MaxWidth = MaxWidth.Small, FullWidth = true, DisableBackdropClick = true };
            var dialog = _dialogService.Show<AddEditRoleModal>(id == null ? "Create" : "Edit", parameters, options);
            var result = await dialog.Result;
            if (!result.Cancelled)
            {
                await Reset();
